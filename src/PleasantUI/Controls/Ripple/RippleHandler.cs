...
internal class RippleHandler : CompositionCustomVisualHandler
{
    ...
    public override void OnRender(ImmediateDrawingContext drawingContext)
    {
        ...
        double currentRadius = _maxRadius;
        double currentOpacity = _opacity;

        if (_transitions)
        {
            double expandingStep = _easing.Ease((double)_animationElapsed.Ticks / _duration.Ticks);
            currentRadius = _maxRadius * expandingStep;

            if (_secondStepStart is { } secondStepStart)
            {
                double opacityStep = _easing.Ease((double)(_animationElapsed - secondStepStart).Ticks /
